...
def convert_to_python_code(properties, output_stream):
    ...
    for i, prop in enumerate(properties):
        if prop == 'src':
            text = properties['src']
            output_stream.write(f"\twebview.load(QUrl('{text}'))\n")

        if prop == 'name':
            text = properties['name']
            output_stream.write(f"\twebview.setWindowTitle('{text}')\n")

        if prop == 'width':
            text = properties['width']
            output_stream.write(f"\twebview.setFixedWidth({text})\n")

        if prop == 'height':
            text = properties['height']
            output_stream.write(f"\twebview.setFixedHeight({text})\n")

        if prop == 'srcdoc':
            text = properties['srcdoc']
            output_stream.write(f"\twebview.setHtml('{text}')\n")
...
if __name__ == "__main__":    
  lexer = XMLLexer(InputStream('<iframe src="https://www.varzesh3.com" name="Sample" width="500"><iframe text="OK" /></iframe>'))
